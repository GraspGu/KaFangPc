...
&& in_array($extension, $allowedExts))
{
    ...
    else
    {
        ...
        //获取数据measure_id
        $id_sel = "select id from user_measure_photo_opinion where user_id = '$unionid'";
        $id_sel_rs = mysql_query($id_sel);
        $rows = mysql_fetch_assoc($id_sel_rs);
        $edition_id = $rows['id'];
        ...
        //更新path路径
            $path = '../../user_photo/'."$unionid"."$edition_id".@date("hisa").'.'."$ext";//注意变量名不能有非法参数，如冒号：
            $sql_path = substr("$path", 17);

        //将path插入数据库
        $user_photo_update = "UPDATE user_measure_photo_opinion set $_SESSION[user_photo_part] = '$sql_path' where user_id = '$unionid' and id = '$edition_id'";
        ...
        else
        {
            ...
            $url = "http://www.ka-fang.cn:8080/online/edition_assistant_data_input.php";
