...
&& in_array($extension, $allowedExts))
{
    ...
    else
    {
        ...
        $opinion_id = $_SESSION['user_opinion_id'];
...
//      echo "文件临时存储的位置: " . $_FILES["file"]["tmp_name"] . "<br>";
        ...
        //更新path路径
            $path = '../../user_photo/'."$unionid"."$opinion_id".@date("hisa").'.'."$ext";//注意变量名不能有非法参数，如冒号：
            $sql_path = substr("$path", 17);
        //将path插入数据库
        $user_photo_update = "UPDATE user_shirt_opinion_list set $_SESSION[des_part] = '$sql_path' where user_id = '$unionid' and shirt_opinion_id = '$opinion_id'";
